...
export class ScriptshahApp extends App implements IPostMessageSent {
    ...
    public async executePostMessageSent(
        ...
        if (!this.isMentionForMe(message.text) || message.sender.username === MY_USERNAME) {
            return;
        }
    ...
    private isMentionForMe(text?: string): boolean {
        if (!text) {
            return false;
        }

        const mentionPattern = new RegExp(`(^|\\s)@${MY_USERNAME}\\b`);
